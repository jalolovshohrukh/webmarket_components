  ...
  theme: {
    ...
    extend: {
      ...
      keyframes: {
        ...
        // Centered modal — preserves translate(-50%, -50%) while it scales,
        // so the panel grows from its own center instead of drifting toward
        // the bottom-right (which is what tailwindcss-animate's zoom-in-95
        // does when combined with a translate-based centering).
        "modal-in": {
        ...
        // Command palette — only X is translated (Y is anchored).
        "palette-in": {
        ...
        "palette-out": {
          ...
          to: { opacity: "0", transform: "translateX(-50%) scale(0.97)" },
        },
      },
      animation: {
        ...
        "modal-out": "modal-out 160ms ease-in",
        "palette-in": "palette-in 180ms cubic-bezier(0.16, 1, 0.3, 1)",
        "palette-out": "palette-out 140ms ease-in",
      },
    },
  },
  plugins: [animate],
