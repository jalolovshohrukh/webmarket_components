...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background p-5 sm:p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300 flex flex-col",
  {
    variants: {
      side: {
  ...
  <SheetPortal>
    ...
    >
      ...
      <DialogPrimitive.Close
        aria-label="Close"
        className="absolute right-4 top-4 rounded-md p-1 text-text-tertiary hover:text-text-primary hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
      >
        <X className="size-5" />
      </DialogPrimitive.Close>
    </DialogPrimitive.Content>
